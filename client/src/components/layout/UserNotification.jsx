...
import { Bell, Package, Clock, Trash2, Volume2, VolumeX, Settings, CheckCircle, Truck, X } from 'lucide-react';
...
const UserNotification = () => {
  ...
  const soundEnabled = soundData?.preferences?.sound?.enabled ?? true;
  const soundType = soundData?.preferences?.sound?.type ?? 'beep';
  ...
  const updateSoundPreferencesMutation = useMutation(
    ...
    {
      onSuccess: () => {
        queryClient.invalidateQueries('userSoundPreferences');
  ...
  useEffect(() => {
    if (!socket) {
      console.log('UserNotification: No socket available');
      return;
    }

    console.log('UserNotification: Setting up socket listeners');

    // Listen for order updates
    socket.on('orderUpdate', (data) => {
      console.log('UserNotification: Received orderUpdate event:', data);
    ...
    return () => {
      console.log('UserNotification: Cleaning up socket listeners');
  ...
  const toggleSound = () => {
    ...
    updateSoundPreferencesMutation.mutate({
      sound: {
